(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))c(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const u of i.addedNodes)u.tagName==="LINK"&&u.rel==="modulepreload"&&c(u)}).observe(document,{childList:!0,subtree:!0});function o(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function c(n){if(n.ep)return;n.ep=!0;const i=o(n);fetch(n.href,i)}})();function I(){return`
    ...
    `}const $="red";let T=0,h=!1;function w(e){const t=document.querySelector("#input-length");t.innerHTML=e,e>T?t.style.color=$:t.style.color=""}function x(e){if(!e)return;let t=e.value;h&&(t=t.replace(/\n/g,"")),w(t.length)}function O(e,t){T=t,x(e)}function M(e,t){if(!e)return;const o=document.querySelector("#input-max-length");o.addEventListener("input",()=>{O(e,o.value)}),e.addEventListener("input",()=>{x(e)}),x(e),h=t}function E(e){e&&x(e)}function R(e,t){h=t,E(e)}function k(e){e&&localStorage.setItem("text",e.value)}function B(e){const t=localStorage.getItem("text");t&&e&&e(t)}function D(e,t){e&&(B(t),e.addEventListener("input",()=>{k(e)}))}function j(e){e&&k(e)}function A(){return`
    <p id="line-navigation"></p>
    `}function H(e){if(!e)return;const t=document.querySelector("#line-navigation"),o=n=>{t.innerHTML=n};(()=>{e.addEventListener("input",()=>{const n=e.value.split(`
...
`).length} / ${o.length}`;t.innerHTML=n}function J(e,t){if(!e)return;const o=document.querySelector("#line-navigation"),c=e.value.split(`
`),i=`行: ${e.value.substr(0,e.selectionStart).split(`
`).length} / ${c.length} ${t}`;o.innerHTML=i}function P(){return`
        <div id="data-io">
...
`))})}function Y(){return`
  ...
  `}function Z(){return`
    ...
    `}function V(e,t){if(!e)return;document.querySelector("#replace-text__button").addEventListener("click",()=>{const u=document.querySelector("#replace-text-dialog");u&&u.showModal()});const c=document.querySelector("#replace-text-dialog");c.addEventListener("close",u=>{const r=u.target.returnValue;r&&t&&t(r)}),document.querySelector("#replace-text-dialog__button").addEventListener("click",()=>{const u=document.querySelector("#replace-text-dialog__input-before"),r=document.querySelector("#replace-text-dialog__input-after"),a=u.value,s=r.value,v=e.value;if(document.querySelector("#replace-text-dialog__case-all").checked){const d=v.replace(new RegExp(a,"g"),s);if(!c)return;c.close(d)}else{const d=v.replace(a,s);if(!c)return;c.close(d)}}),document.querySelector("#replace-text-dialog__cancel").addEventListener("click",()=>{c&&c.close()})}function ee(){return`
        <div id="ignore-newline-dialog">
            <button id="ignore-newline-dialog__button">改行無視</button>
        </div>
    `}let b=!1;function te(){return b}function ne(e,t){if(!e)return;const o=document.querySelector("#ignore-newline-dialog__button");o.addEventListener("click",()=>{b?(o.style.backgroundColor="#000000",o.style.color="#7eeea8"):(o.style.backgroundColor="#13bb8e",o.style.color="#dffff1"),b=!b,t&&t(e,b)})}function ie(){return`
    ...
    `}function re(e,t,o,c){if(!e)return;let n=e.value;const i=document.querySelector("#increment-decrement-dialog"),u=document.querySelector("#increment-decrement__button"),r=document.querySelector("#increment-decrement-dialog__increment"),a=document.querySelector("#increment-decrement-dialog__decrement"),s=document.querySelector("#increment-decrement-dialog__close"),v=document.querySelector("#increment-decrement-dialog__reset");u.addEventListener("click",()=>{if(!i)return;i.showModal(),o(),n=e.value;const f=document.querySelector("#increment-decrement-dialog__input-start"),d=document.querySelector("#increment-decrement-dialog__input-end");f.value="0",d.value=n.split(`
...
`).map((m,g)=>_<=g+1&&g+1<=S?m.replace(/[+-]?\d+/g,L=>String(Number(L)-1)):m).join(`
`),t.value=n}),s.addEventListener("click",()=>{i&&i.close(n)}),v.addEventListener("click",()=>{i&&i.close()})}function q(){const e=[...document.querySelectorAll("#tool-bar, #information, #line-navigation")].reduce((o,c)=>o+c.clientHeight,0);console.log(e),document.querySelectorAll("textarea").forEach(o=>{o.style.height="auto",o.style.height=document.body.clientHeight-e-8+"px"})}function ce(){window.addEventListener("resize",()=>{q()}),q()}document.querySelector("#app").innerHTML=`
  <div id="tool-bar">
  ...
  ${Z()}
  ${oe()}
`;const l=document.querySelector("#editor"),y=document.querySelector("#dummy-editor");function p(e){e===null?(l.focus(),l.select(),document.execCommand("delete")):e&&(l.focus(),l.select(),document.execCommand("insertText",!1,e)),U(l),j(l),E(l)}function le(e){J(l,z(e))}function ue(){y.value=l.value,l.style.display==="none"?(l.style.display="inline-block",y.style.display="none"):(l.style.display="none",y.style.display="block",y.style.display="inline-block")}D(l,p);M(l,te());H(l);C(l,p);K(l,p,le);X(l,p);Q(l,p);V(l,p);ne(l,R);re(l,y,ue,p);ce();
